...
def is_heading_by_pattern(text):
    """
    Check if text matches common heading patterns (multilingual support)
    """
    text = text.strip()
    if not text:
        return False
    
    # Common heading patterns (multilingual)
    patterns = [
        r'^\d+\.\s+',  # 1. Introduction
        r'^\d+\.\d+\s+',  # 1.1 Subsection
        r'^\d+\.\d+\.\d+\s+',  # 1.1.1 Subsubsection
        r'^Chapter\s+\d+',  # Chapter 1 (English)
        r'^Section\s+\d+',  # Section 1 (English)
        r'^第\d+章',  # 第1章 (Japanese Chapter)
        r'^第\d+節',  # 第1節 (Japanese Section)
        r'^अध्याय\s+\d+',  # अध्याय 1 (Hindi Chapter)
        r'^खंड\s+\d+',  # खंड 1 (Hindi Section)
        r'^[A-Z\u4e00-\u9fff\u0900-\u097f][A-Z\u4e00-\u9fff\u0900-\u097f\s]+$',  # ALL CAPS (multilingual)
        r'^[A-Z\u4e00-\u9fff\u0900-\u097f][a-z\u4e00-\u9fff\u0900-\u097f\s]+:$',  # Title Case with colon (multilingual)
        r'^\w+[。：]',  # Japanese/Chinese punctuation endings
        r'^.*[।:]$',  # Hindi punctuation endings
    ]
...
def determine_heading_level(text, font_size, font_flags, avg_font_size, font_size_levels):
    """
    Determine heading level based on font size, style, and text patterns (multilingual)
    """
    # Check for explicit numbering patterns (multilingual)
    ...
    # Check for chapter/section patterns (multilingual)
    if re.match(r'^(Chapter|Section|第\d+章|第\d+節|अध्याय|खंड)\s*\d*', text, re.IGNORECASE | re.UNICODE):
        return "H1"
    
    # Japanese/Chinese heading patterns
    if re.match(r'^第\d+章', text):  # 第1章
        return "H1"
    elif re.match(r'^第\d+節', text):  # 第1節
        return "H2"
    
    # Hindi heading patterns
    if re.match(r'^अध्याय\s+\d+', text):  # अध्याय 1
        return "H1"
    elif re.match(r'^खंड\s+\d+', text):  # खंड 1
        return "H2"
    ...
    for page in pages:
        ...
        for block in page["blocks"]:
            ...
            if is_heading:
                ...
                if level:
                    # Clean up heading text (multilingual support)
                    clean_text = re.sub(r'^\d+\.\s*', '', text)  # Remove numbering
                    clean_text = re.sub(r'^\d+\.\d+\s*', '', clean_text)
                    clean_text = re.sub(r'^\d+\.\d+\.\d+\s*', '', clean_text)
                    # Remove Japanese/Chinese chapter/section markers
                    clean_text = re.sub(r'^第\d+章\s*', '', clean_text)
                    clean_text = re.sub(r'^第\d+節\s*', '', clean_text)
                    # Remove Hindi chapter/section markers
                    clean_text = re.sub(r'^अध्याय\s+\d+\s*', '', clean_text)
                    clean_text = re.sub(r'^खंड\s+\d+\s*', '', clean_text)
